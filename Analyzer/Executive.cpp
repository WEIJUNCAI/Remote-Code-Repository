...
void showUsage()
{
  ...
  out << "\n    ***********************************************************************";
  out << "\n    - p : followed by a path to store";
  out << "\n          all generated HTML version of the analyzed source files,";
  out << "\n          the default path is the \"PublishedFiles\" folder";
...
{
	std::cout << "\n  Processing command line:" << std::endl;
	showUsage();
...
void CodeAnalysis::CodeAnalysisExecutive::processCPP(bool showProc)
{
...
void CodeAnalysis::CodeAnalysisExecutive::processH(bool showProc)
{
...
void CodeAnalysis::CodeAnalysisExecutive::processCS(bool showProc)
{
	for (auto file : csharpFiles())
	{
		...
		depAnaly.AddNewFile(file, "C# file", "package");
		Slocs slocs = pRepo_->Toker()->currentLineCount();
		slocMap_[pRepo_->package()] = slocs;
	}
}
...
void CodeAnalysis::CodeAnalysisExecutive::analyzeCPP()
{
	for (auto file : cppImplemFiles())
	{
		pRepo_->package() = FileSystem::Path::getName(file);
...
void CodeAnalysis::CodeAnalysisExecutive::analyzeH()
{
	for (auto file : cppHeaderFiles())
	{
...
void CodeAnalysis::CodeAnalysisExecutive::analyzeCS()
{
	for (auto file : csharpFiles())
	{
		pRepo_->package() = FileSystem::Path::getName(file);
...
void CodeAnalysis::CodeAnalysisExecutive::PublishFiles()
{
...
void CodeAnalysis::CodeAnalysisExecutive::OpenFile(std::string& path)
{
...
void CodeAnalysis::CodeAnalysisExecutive::OpenHTMLsInBrowser()
{
	for (auto & file : publisher.getHTMLfiles())
		OpenFile(file);
}

void CodeAnalysis::CodeAnalysisExecutive::DemonstrateP3()
{
	std::cout << "\n\n==========================================================\n";
	std::cout << "\nDemonstrating the CSS style sheet:" << std::endl;
	std::cout << "\n<--------------------------------------------------------->\n";

	std::ifstream ifs;
	std::string line;
	ifs.open(FileSystem::Path::getFullFileSpec(stylePath), std::ifstream::in);
	...
	std::cout << "\n\n==========================================================\n";
	std::cout << "\nDemonstrating the JavaScript file:" << std::endl;
	std::cout << "\n<--------------------------------------------------------->\n";
	ifs.open(FileSystem::Path::getFullFileSpec(jsPath), std::ifstream::in);
	...
	std::cout << "\n\n==========================================================\n";
	std::cout << "\nDemonstrating a sample HTML file:" << std::endl;
	std::cout << "\n<--------------------------------------------------------->\n";
	ifs.open(publisher.getHTMLfiles()[0], std::ifstream::in);
	while (getline(ifs, line).good())
		std::cout << line << '\n';
	ifs.close();
	std::cout << "\n\n==========================================================\n";
	std::cout << "\nDemonstrating all published important packages of this project in browser (scope collapse supported):\n" << std::endl;
	OpenHTMLsInBrowser();
}
...
void CodeAnalysisExecutive::displayMetricSummary(size_t sMax, size_t cMax)
{
  ...
  if (fileNodes_.size() == 0)  // only build fileNodes_ if displayMetrics hasn't been called
  {
    std::function<void(ASTNode* pNode)> co = [&](ASTNode* pNode) {
      fileNodes_.push_back(std::pair<File, ASTNode*>(pNode->package_, pNode));
    }; // function invoked on every node to add the node to fileNodes (key: file | value: a AST Node in this file)
  ...
  for (auto item : fileNodes_)
  {
    if (item.second->type_ == "function")
    {
      ...
      if (size > sMax || cmpl > cMax) // checking if function exceed limits
        displayMetricsLine(item.first, item.second);
    }
  }
  Rslt::write("\n");
  for (auto item : fileNodes_)
  {
    displayDataLines(item.second, true);  // identifying and displaying public data
  }
  Rslt::write("\n");
}
