...
void ClientHandler::handleFileMsg(Message & msg, const std::string& filename, Socket & socket)
{
	...
	if (!readFile(filename, category, errorMsg, contentSize, socket))
	{
...
*/
bool ClientHandler::readFile(const std::string& filename, const std::string& category, std::string& errorMsg, size_t fileSize, Socket& socket)
{
	bool isSuccessful = true;
	...
	std::string parentDir = (category == "NULL")? rootFilePath : rootFilePath + "\\" + category;
	std::string fqname = parentDir + "\\" + filename;
